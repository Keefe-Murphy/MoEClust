...
#' @param ... Catches unused arguments, or arguments to be passed to any/all of \code{scatter.pars, barcode.pars, mosaic.pars, axis.pars} and \code{diag.pars}.
#'
...
#' MoE_gpairs(res, conditional=c("stripplot", "barcode"),
#'            scatter.type=c("ci", "points"), response.type="uncertainty",
#'            hist.color=c("black", "cyan", "navy", "hotpink"),
#'            bg.col=c("oldlace", "white", "seashell", "whitesmoke", "cornsilk"))
MoE_gpairs          <- function(res, response.type = c("points", "uncertainty", "density"), scatter.type = c("lm", "points"), conditional = c("stripplot", "boxplot"), addEllipses = c("outer", "yes", "no", "inner"),
                                bg.col = c("ivory", "white", "floralwhite", "floralwhite", "cornsilk"), diagonal = c("on", "off"), outer.margins = list(bottom=unit(2, "lines"), left=unit(2, "lines"), top=unit(2, "lines"), right=unit(2, "lines")),
                                outer.labels = NULL, outer.rot = c(0, 90), gap = 0.05, buffer = 0.02, scatter.pars = list(...), stripplot.pars = list(...), barcode.pars = list(...), mosaic.pars = list(...), axis.pars = list(...), diag.pars = list(...), ...) {
  UseMethod("MoE_gpairs")
}

#' @method MoE_gpairs MoEClust
#' @export
MoE_gpairs.MoEClust <- function(res, response.type = c("points", "uncertainty", "density"), scatter.type = c("lm", "points"), conditional = c("stripplot", "boxplot"), addEllipses = c("outer", "yes", "no", "inner"),
                                bg.col = c("ivory", "white", "floralwhite", "floralwhite", "cornsilk"), diagonal = c("on", "off"), outer.margins = list(bottom=unit(2, "lines"), left=unit(2, "lines"), top=unit(2, "lines"), right=unit(2, "lines")),
                                outer.labels = NULL, outer.rot = c(0, 90), gap = 0.05, buffer = 0.02, scatter.pars = list(...), stripplot.pars = list(...), barcode.pars = list(...), mosaic.pars = list(...), axis.pars = list(...), diag.pars = list(...), ...) {
  ...
  uncertainty  <- 0.25   * (uncertainty - min(uncertainty))/diff(range(uncertainty))
    ...
    for(j in Nseq) {
      ...
      if(i == j) {
        ...
        if(is.factor(x[,i]) + is.factor(x[,j]) == 1) {
          if(i < j & upr.cond != "barcode") .boxplot_panel(x=x[,j], y=x[,i], type=upr.cond, axis.pars=axis.pars, xpos=xpos, ypos=ypos, buffer=buffer, stripplot.pars=stripplot.pars, outer.rot=outer.rot, bg=bg, box.fill=if(i == 1 || j == 1) colors else if(i > dcol && j > dcol) "grey" else NULL)
          if(i > j & low.cond != "barcode") .boxplot_panel(x=x[,j], y=x[,i], type=low.cond, axis.pars=axis.pars, xpos=xpos, ypos=ypos, buffer=buffer, stripplot.pars=stripplot.pars, outer.rot=outer.rot, bg=bg, box.fill=if(i == 1 || j == 1) colors else if(i > dcol && j > dcol) "grey" else NULL)
        ...
        if(!any(is.factor(x[,i]), is.factor(x[,j]))) {
          .scatter_panel(x=x[,j], y=x[,i], type=ifelse(j > dcol && i <= dcol, upr.exp, ifelse(j <= dcol && i <= dcol, ifelse(drawEllipses, "ellipses", "points"), low.exp)),
                         ...
                         bg=bg, uncertainty=if(response.type == "uncertainty" && (i <= dcol && j <= dcol)) uncertainty else NA, mvn.col=if(colEllipses) scatter.pars$lcol else NULL, mvn.type = addEllipses)
        }
...
.draw_axis <- function(x, y, axis.pars, xpos, ypos, cat.labels = NULL, horiz = NULL, xlim = NULL, ylim = NULL, outer.rot) {
  x        <- as.numeric(x)
  y        <- as.numeric(y)
  x.fac    <- all(x == floor(x)) && length(unique(x)) < 5
  y.fac    <- all(y == floor(y)) && length(unique(y)) < 5
  if(is.null(xlim)) {
    px     <- if(x.fac) seq_along(unique(x))  else pretty(x,    axis.pars$n.ticks)
    px     <- px[px > min(x, na.rm=TRUE)    & px < max(x, na.rm=TRUE)]
  } else {
    px     <- if(x.fac) seq_along(unique(x))  else pretty(xlim, axis.pars$n.ticks)
    px     <- px[px > min(xlim, na.rm=TRUE) & px < max(xlim, na.rm=TRUE)]
  }
  if(is.null(ylim)) {
    py     <- if(y.fac) seq_along(unique(y))  else pretty(y,    axis.pars$n.ticks)
    py     <- py[py > min(y, na.rm=TRUE)    & py < max(y, na.rm=TRUE)]
  } else {
    py     <- if(y.fac) seq_along(unique(y))  else pretty(ylim, axis.pars$n.ticks)
...
.scatter_panel <- function(x, y, type, scatter.pars, axis.pars, xpos, ypos, buffer, z, G, res, dimens, outer.rot, bg, uncertainty, mvn.col, mvn.type) {
