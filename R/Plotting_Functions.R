...
#' All of the above are relevant for \code{"boxplot"} panels, are passed to \code{\link[lattice]{panel.bwplot}} when producing boxplots, and retain the same definitions as the similarly named arguments therein. However, only \code{box.col}, \code{varwidth}, and \code{box.fill} are relevant for \code{"violin"} panels, and in both cases \code{box.fill} is only invoked for panels where the categorical variable is the MAP classification (i.e. when \code{subset$show.map=TRUE}). See \code{diag.pars$hist.color} for controlling the colours of non-MAP-related boxplot/violin panels. Notably, \code{box.fill} will inherit a suitable default from \code{scatter.pars$scat.col} if the latter is supplied but the former is not.
#' @param barcode.pars A list supplying select parameters for continuous vs. categorical panels when one or both of the entries of \code{conditional} is \code{"barcode"}.
#'
#' \code{NULL} is equivalent to:
...
#' @param ... Catches unused arguments. Alternatively, named arguments can be passed directly here to any/all of \code{scatter.pars}, \code{density.pars}, \code{diag.pars}, \code{stripplot.pars}, \code{boxplot.pars}, \code{barcode.pars}, \code{mosaic.pars}, and \code{axis.pars}.
#'
...
MoE_gpairs          <- function(res, response.type = c("points", "uncertainty", "density"), subset = list(...), scatter.type = c("lm", "points"), conditional = c("stripplot", "boxplot"),
                                ...
                                scatter.pars = list(...), density.pars = list(...), diag.pars = list(...), stripplot.pars = list(...), boxplot.pars = list(...), barcode.pars = list(...), mosaic.pars = list(...), axis.pars = list(...), ...) {
...
MoE_gpairs.MoEClust <- function(res, response.type = c("points", "uncertainty", "density"), subset = list(...), scatter.type = c("lm", "points"), conditional = c("stripplot", "boxplot"),
 ...
 !all(subset$cov.ind  %in% c(0, seq_len(ncol(net))))) stop("Invalid 'subset$cov.ind'",  call.=FALSE)
  ...
  densities             <- vector("list", dcol - subset$show.map)
  ...
  res$parameters$varianceX        <- if(isTRUE(expert.covar)  &&
                                        attr(res, "Expert")   &&
                                        isTRUE(drawEllipses)) suppressMessages(expert_covar(res, ...)) else res$parameters$variance
  ...
  barcode.pars$colbar   <- rev(barcode.pars$col)[claSS]
  if(is.null(barcode.pars$nint))     {
    barcode.pars$nint   <- 0
  }
  if(is.null(barcode.pars$ptsize))   {
    barcode.pars$ptsize <- scatter.pars$size
  }
  if(is.null(barcode.pars$ptpch))    {
    barcode.pars$ptpch  <- scatter.pars$scat.pch
  }
  ...
  for(i   in Nseq) {
    for(j in Nseq) {
      ...
      if(i == j) {
        ...
        .diag_panel(x=x[,i], varname=names(x)[i], diag.pars=diag.pars, hist.col=if(i == 1 && names(x)[i] == "MAP") list(noise.cols) else diag.pars$hist.color, axis.pars=axis.pars, xpos=xpos, ypos=ypos, buffer=buffer, index=i, i=subset$data.ind[i - subset$show.map], res=res, col=noise.cols, outer.rot=outer.rot)
      } else  {
          ...
          if(i < j & upr.cond == "barcode") {
            ...
            if(is.factor(x[,i])) {
              .bar_code(x=split(x[,j], x[,i])[length(levels(x[,i])):1L], horizontal=TRUE, xlim=NULL, labelloc=ypos, axisloc=xpos, labelouter=TRUE, 
                        newpage=FALSE, fontsize=axis.pars$fontsize, buffer=buffer, nint=barcode.pars$nint, bcspace=barcode.pars$bcspace, outerbox=border,
                        ptsize=if(barcode.pars$use.points && length(barcode.pars$ptsize) > 1) rev(split(barcode.pars$ptsize, x[,i])) else barcode.pars$ptsize, 
                        ptpch=if(barcode.pars$use.points  && length(barcode.pars$ptpch)  > 1) rev(split(barcode.pars$ptpch,  x[,i])) else barcode.pars$ptpch,  
                        col=if(isTRUE(MAPpan)) barcode.pars$col else rev(split(barcode.pars$colbar, x[,i])), use.points=barcode.pars$use.points)
            } else {
              ...
              .bar_code(x=split(x[,i], x[,j])[length(levels(x[,j])):1L], horizontal=FALSE, xlim=NULL, labelloc=xpos, axisloc=ypos, labelouter=TRUE, 
                        newpage=FALSE, fontsize=axis.pars$fontsize, buffer=buffer, nint=barcode.pars$nint, bcspace=barcode.pars$bcspace, outerbox=border,
                        ptsize=if(barcode.pars$use.points && length(barcode.pars$ptsize) > 1) rev(split(barcode.pars$ptsize, x[,j])) else barcode.pars$ptsize, 
                        ptpch=if(barcode.pars$use.points  && length(barcode.pars$ptpch)  > 1) rev(split(barcode.pars$ptpch,  x[,j])) else barcode.pars$ptpch,  
                        col=if(isTRUE(MAPpan)) barcode.pars$col else rev(split(barcode.pars$colbar, x[,j])), use.points=barcode.pars$use.points)
            }
            grid::popViewport()
          }
        ...
        if(!any(is.factor(x[,i]), is.factor(x[,j])))  {
          ...
          if(response.type  == "density" && all(j <= dcol, i <= dcol)) {
            densities[[i - subset$show.map]][[j - subset$show.map]] <- .density_panel(cbind(x[,j], x[,i]), dimens=c(subset$data.ind[j - subset$show.map], subset$data.ind[i - subset$show.map]), res, density.pars, axis.pars, xpos, ypos, buffer, outer.rot, bg, border, scatter.pars, zz=if(submat == "all" & i > j) densities[[j - subset$show.map]][[i - subset$show.map]])
          } else {
...
MoE_Uncertainty          <- function(res, type = c("barplot", "profile"), truth = NULL, decreasing = FALSE, 
                                     col = c("cluster", "uncertain", "none"), rug1d = TRUE, ...) {
...
MoE_Uncertainty.MoEClust <- function(res, type = c("barplot", "profile"), truth = NULL, decreasing = FALSE, 
  ...
  if(!missing(col)       && (length(col)  > 1     ||
     !is.character(col)))                             stop("'col' must be a single character string",         call.=FALSE)
  type       <- match.arg(type)
  col        <- match.arg(col)
  n.obs      <- res$n
  ...
  if(type    == "barplot")   {
    cu       <- if(tmiss) replace(rep(cm[1L], n.obs), mC, cm[2L]) else switch(EXPR=col, cluster=replace(cm[res$classification], res$classification == 0, "darkgrey"), uncertain=cm[seq_len(2L)][(ucert >= oneG) + 1L], rep("black", n.obs))
    cu[ucert == 0] <- NA
    if(res$d == 1  && isTRUE(rug1d)) {
      x      <- as.matrix(res$data)
      base::plot(x, ucert, type="h", ylim=range(yx), col=cu, yaxt="n", ylab="", lend=1, xlab=colnames(res$data))
      graphics::lines(x=graphics::par()$usr[1L:2L], y=c(oneG, oneG), lty=2, col="black")
      graphics::rug(x)
    } else          {
      base::plot(ucert,    type="h", ylim=range(yx), col=cu, yaxt="n", ylab="", lend=1, xlab="Observations")  
      graphics::lines(x=c(0, n.obs),                y=c(oneG, oneG), lty=2, col="black")
    }
    ...
    graphics::points(ucord, pch=15, cex=if(tmiss) replace(rep(0.5, n.obs), mcO, 0.75) else 0.5, col=if(tmiss) replace(rep(1, n.obs), mcO, cm[2L]) else switch(EXPR=col, cluster=replace(cm[res$classification], res$classification == 0, "darkgrey")[ord], uncertain=cm[seq_len(2L)][(ucord >= oneG) + 1L], "black"))
    graphics::lines(x=c(0, n.obs), y=c(oneG, oneG), lty=2, col="black")
    graphics::axis(2, at=yx, labels=replace(yx, YX, ifelse(noise, expression(1 - frac(1, widehat(G^{'(0)'}))), expression(1 - frac(1, hat(G))))), las=2, xpd=TRUE)
    graphics::axis(2, at=oneG, labels=ifelse(noise, expression(frac(1, widehat(G^{'(0)'}))), expression(frac(1, hat(G)))), las=2, xpd=TRUE, side=4)
    if(tmiss) {
...
#' @importFrom vcd "strucplot"
#' @note Other plotting options are available by first calling \code{\link[=as.Mclust.MoEClust]{as.Mclust}} on the fitted object, and then calling \code{\link[mclust]{plot.Mclust}} on the results. However, caution is advised for models with gating &/or expert covariates (see the Note in \code{\link[=as.Mclust.MoEClust]{as.Mclust}}).
#' @return The visualisation according to \code{what} of the results of a fitted \code{MoEClust} model.
#' @seealso \code{\link{MoE_clust}}, \code{\link{MoE_stepwise}}, \code{\link{MoE_gpairs}}, \code{\link{MoE_plotGate}}, \code{\link{MoE_plotCrit}}, \code{\link{MoE_plotLogLik}}, \code{\link{MoE_Similarity}}, \code{\link{MoE_Uncertainty}}, \code{\link[=as.Mclust.MoEClust]{as.Mclust}}, \code{\link[mclust]{plot.Mclust}}
#' @references Murphy, K. and Murphy, T. B. (2020). Gaussian parsimonious clustering models with covariates and a noise component. \emph{Advances in Data Analysis and Classification}, 14(2): 293-325. <\doi{10.1007/s11634-019-00373-8}>.
#' @author Keefe Murphy - <\email{keefe.murphy@@mu.ie}>
#' @export
...
.draw_axis <- function(x, y, axis.pars, xpos, ypos, cat.labels = NULL, horiz = NULL, xlim = NULL, ylim = NULL, outer.rot) {
  ...
  if(!is.null(xpos)) {
    if(!is.null(cat.labels) && isFALSE(horiz)) {
      grid::grid.text(cat.labels, x=grid::unit(seq_len(k), "native"), y=grid::unit(rep(1 * (1 - xpos), k), "npc") + grid::unit(rep(-1 * xpos + 1 * (1 - xpos), k), "lines"), gp=grid::gpar(fontsize=axis.pars$fontsize), rot=outer.rot[1L])
  ...
  if(!is.null(ypos)) {
    if(!is.null(cat.labels) && isTRUE(horiz))  {
      grid::grid.text(cat.labels, y=grid::unit(seq_len(k), "native"), x=grid::unit(rep(1 * (1 - ypos), k), "npc") + grid::unit(rep(-1 * ypos + 1 * (1 - ypos), k), "lines"), gp=grid::gpar(fontsize=axis.pars$fontsize), rot=outer.rot[2L])
...
#' @importFrom matrixStats "colMeans2" "rowLogSumExps"
.density_panel <- function(dat, dimens, res, density.pars, axis.pars, xpos, ypos, buffer, outer.rot, bg, border, scatter.pars, zz = NULL) {
  pars         <- res$parameters
  ...
  if(is.element(modelName, c("EII", "VII", "EEI")))           {
    ...
    sigma      <- c(sigma, list(sigma=pars$variance$sigma[dimens,dimens,, drop=FALSE]))
  }
  ...
  if((z.null   <- is.null(zz)))      {
    ...
    if(expx)          {
      if(gate)        {
        d      <- lapply(seq_len(n), function(i) MoE_dens(data=xy, mus=mu[i,,], sigs=sigma, log.tau=pars$lpro[i,], Vinv=Vinv))
      } else d <- lapply(seq_len(n), function(i) MoE_dens(data=xy, mus=mu[i,,], sigs=sigma))
      d        <- .log_avg_den(d)
    }   else if(gate) {
      d        <- t(MoE_dens(data=xy, mus=mu, sigs=sigma, Vinv=Vinv))
      d        <- t(d + log(colMeans2(pars$pro, refine=FALSE, useNames=FALSE)))
    }   else d <- MoE_dens(data=xy,   mus=mu, sigs=sigma)
    if(isFALSE(gate)) {
      d        <- if(isTRUE(noise))            cbind(d, log(Vinv)) else d
      d        <- .logsumexp(d, pars$lpro)
    }   else d <- .logsumexp(d)
    zz         <- matrix(exp(d), nrow=lx, ncol=ly, byrow=FALSE)
  }
  ...
  grid::grid.rect(gp=grid::gpar(fill=bg, col=border))
  .contour_panel(x, y, if(z.null) zz else t(zz), density.pars)
...
.diag_panel <- function(x, varname, diag.pars, hist.col, axis.pars, xpos, ypos, buffer, index, i, res, col, outer.rot) {
  ...
  if(diag.pars$show.dens) {
    if(!is.factor(x)) {
      ...
      pars         <- res$parameters
      modelName    <- pars$variance$modelName
      G            <- res$G
      n            <- res$n
      ...
      if(is.element(modelName, c("EII", "VII", "EEI", "EEE", "E", "V"))) {
        ...
        sigma      <- c(sigma, list(sigmasq=pars$variance$sigma[i,i,, drop=FALSE]))
        sigma$modelName  <- "V"
      }
...
.bar_code       <- function(x, outer.margins = list(bottom = grid::unit(2, "lines"), left = grid::unit(2, "lines"), top = grid::unit(2, "lines"), right = grid::unit(2, "lines")),
  ...
  .barcode_panel(x, horizontal=horizontal, nint=nint, xlim=xlim, labelloc=labelloc, labelouter=labelouter,
                 fontsize=fontsize, ptsize=ptsize, ptpch=ptpch, bcspace=bcspace, use.points=use.points, 
                 xlab=xlab, xlaboffset=xlaboffset, axisloc=axisloc, buffer=buffer, log=log, col=col)
  grid::popViewport(1)
...
.barcode_panel  <- function(x, horizontal = TRUE, xlim = NULL, labelloc = TRUE, axisloc = TRUE, labelouter = FALSE,
  ...
  for(i in seq_len(K))  {
    ...
    if(length(mids) > 0) {
      ...
      grid::grid.segments(grid::unit(mids[z > 0], "native"), 0, gp=grid::gpar(col=if(inherits(col, "list")) col[[i]] else col[i]), grid::unit(mids[z > 0], "native"), 1)
      grid::popViewport(1)
      ...
      for(j in seq_along(z)) {
        if(z[j] > 1) {
          ...
          if(use.points) {
            uord <- order(y[!duplicated(y)])
            grid::grid.points(grid::unit(xx, "native"), yy, 
                              size=if(!inherits(ptsize, "list")) ptsize else rep(rep(ptsize[[i]][uord], z)[j], z[j] - 1), 
                              pch=if(!inherits(ptpch,   "list")) ptpch  else rep(rep(ptpch[[i]][uord],  z)[j], z[j] - 1), 
                              gp=grid::gpar(col=if(inherits(col, "list"))    rep(rep(col[[i]][uord],    z)[j], z[j] - 1)  else col[i]))
          } else  {
            yy   <- if(log) c(yy, log(2L + z[j])/maxct)        else c(yy, (z[j])/maxct)
            grid::grid.segments(grid::unit(mids[j], "native"), gp=grid::gpar(col=if(inherits(col, "list")) col[[i]]       else col[i]), grid::unit(1/maxct, "npc"), grid::unit(mids[j], "native"), grid::unit(max(yy), "npc"))
          }
        }
      }
      grid::popViewport(2)
