...
#' # Add confidence intervals to the scatter plots.
...
MoE_gpairs          <- function(res, response.type = c("points", "uncertainty", "density"), residuals = FALSE, scatter.type = c("lm", "points"), conditional = c("stripplot", "boxplot"),
...
#' @export
MoE_gpairs.MoEClust <- function(res, response.type = c("points", "uncertainty", "density"), residuals = FALSE, scatter.type = c("lm", "points"), conditional = c("stripplot", "boxplot"),
                                addEllipses = c("outer", "yes", "no", "inner"), border.col = c("purple", "black", "brown", "brown", "navy"), bg.col = c("cornsilk", "white", "whitesmoke", "whitesmoke", "cornsilk"),
                                diagonal = c("on", "off"), outer.margins = list(bottom=unit(2, "lines"), left=unit(2, "lines"), top=unit(2, "lines"), right=unit(2, "lines")), outer.labels = NULL, outer.rot = c(0, 90),
                                gap = 0.05, buffer = 0.02, scatter.pars = list(...), density.pars = list(...), stripplot.pars = list(...), barcode.pars = list(...), mosaic.pars = list(...), axis.pars = list(...), diag.pars = list(...), ...) {
  ...
  class <- factor(class)
  x     <- cbind(MAP=class, dat, if(resid) do.call(rbind, replicate(G, net, simplify=FALSE)) else net)
  ...
  if(is.null(density.pars$grid.size)) {
    density.pars$grid.size   <- c(100, 100)
            ...
            !all(density.pars$grid.size      < 10))   stop("Invalid 'density.pars$grid.size'")
  ...
  for(i   in Nseq) {
    for(j in Nseq) {
      ...
      border <- if(i == 1 || j == 1) border.col[1] else if(i <= dcol && j <= dcol) border.col[2] else if(i > dcol && j > dcol) border.col[5] else if(j > dcol && i <= dcol) border.col[3] else border.col[4]
      ...
      if(i == j) {
        ...
        if(is.factor(x[,i]) + is.factor(x[,j]) == 1) {
          if(i < j & upr.cond != "barcode") .boxplot_panel(x=x[,j], y=x[,i], type=upr.cond, axis.pars=axis.pars, xpos=xpos, ypos=ypos, buffer=buffer, stripplot.pars=stripplot.pars, outer.rot=outer.rot, bg=bg, box.fill=if(i == 1 || j == 1) noise.cols else if(i > dcol && j > dcol) "grey" else NULL, border=border)
          if(i > j & low.cond != "barcode") .boxplot_panel(x=x[,j], y=x[,i], type=low.cond, axis.pars=axis.pars, xpos=xpos, ypos=ypos, buffer=buffer, stripplot.pars=stripplot.pars, outer.rot=outer.rot, bg=bg, box.fill=if(i == 1 || j == 1) noise.cols else if(i > dcol && j > dcol) "grey" else NULL, border=border)
        ...
        if(!any(is.factor(x[,i]), is.factor(x[,j])))  {
          if(response.type == "density" && all(j <= dcol, i <= dcol)) {
            .density_panel(cbind(x[,j], x[,i]), dimens=c(j - 1, i - 1), res, density.pars, axis.pars, xpos, ypos, buffer, outer.rot, bg, border, resid)
          } else {
            .scatter_panel(x=x[,j], y=x[,i], type=ifelse(j > dcol && i <= dcol, upr.exp, ifelse(j <= dcol && i <= dcol, ifelse(drawEllipses, "ellipses", "points"), low.exp)), scatter.pars=scatter.pars,
                           axis.pars=axis.pars, xpos=xpos, ypos=ypos, buffer=buffer, z=z, G=G, res=res, dimens=c(j - 1, i - 1), outer.rot=outer.rot, bg=bg, mvn.type=addEllipses, border=border,
...
.density_panel <- function(dat, dimens, res, density.pars, axis.pars, xpos, ypos, buffer, outer.rot, bg, border, residuals) {
  ...
  xy           <- .grid_2(x, y)
  Vnul         <- is.null(res$parameters$Vinv)
  ltau         <- .mat_byrow(log(if(attr(res, "Gating")) colMeans(res$z) else par$pro), nrow=nrow(xy), ncol=G + !Vnul)
  ltau         <- if(Vnul) ltau else ltau[,-(G + 1)]
