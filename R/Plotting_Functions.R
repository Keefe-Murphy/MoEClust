...
MoE_gpairs.MoEClust <- function(res, response.type = c("points", "uncertainty", "density"), subset = list(...), scatter.type = c("lm", "points"), conditional = c("stripplot", "boxplot"),
 ...
 !all(subset$cov.ind  %in% c(0, seq_len(ncol(net))))) stop("Invalid 'subset$cov.ind'", call.=FALSE)
  ...
  if(is.null(outer.labels)) {
    lab1   <- switch(EXPR=names(x)[1L], MAP=1L, 2L)
  ...
  x[,1]                 <- if(names(x)[1L]  == "MAP") factor(class) else x[,1L]
...
#' @param res An object of class \code{"MoEClust"} generated by \code{\link{MoE_clust}}, or an object of class \code{"MoECompare"} generated by \code{\link{MoE_compare}}. Models with a noise component are facilitated here too.
#' @param x.axis Optional argument for the x-axis against which the mixing proportions are plotted. Defaults to \code{1:res$n} if missing.
...
MoE_plotGate.MoEClust   <- function(res, x.axis = NULL, type = "l", xlab = "Observation", ylab = expression(widehat(tau)[g]), ylim = c(0, 1), col = NULL, ...) {
  ...
  x.axis     <- if(missing(x.axis)) seq_len(N) else x.axis
  cX         <- missing(col)
  nX         <- is.na(res$hypvol)
  col        <- if(cX)                                  seq_len(G) else col
  col        <- if(cX  && !nX) c(rep(col, length.out=G), "grey65") else col
  if(missing(ylab))     {
    graphics::matplot(x=x.axis, y=Tau, type=type, xlab="",   ylab="",   ylim=ylim, col=col, ...)
    dots     <- list(...)
    ...
    graphics::matplot(x=x.axis, y=Tau, type=type, xlab=xlab, ylab=ylab, ylim=ylim, col=col, ...)
  }
    ...
    if(length(mids) > 0) {
      ...
      for(j in seq_along(z)) {
        if(z[j] > 1) {
          ...
          yy     <- if(log) (log(2L + 1L:(z[j] - 1L)))/maxct else (1L:(z[j] - 1L))/maxct
          if(use.points) {
