...
#' @param barcode.pars A list supplying select parameters for continuous vs. categorical panels  when one of the entries of \code{conditional} is \code{"boxplot"}. \code{NULL} is equivalent to \code{list(nint=0, ptsize=unit(0.25, "char"), ptpch=1, bcspace=NULL, use.points=FALSE)}. See \code{\link[barcode]{barcode}}.
...
#'
#' @importFrom barcode "barcode" "barcode.panel"
#' @importFrom grid "convertHeight" "convertWidth" "gpar" "grid.lines" "grid.newpage" "grid.points" "grid.polygon" "grid.rect" "grid.text" "grid.xaxis" "grid.yaxis" "popViewport" "pushViewport" "unit" "upViewport" "viewport"
...
#' @seealso \code{\link{MoE_clust}}, \code{\link{plot.MoEClust}}, \code{\link[lattice]{panel.stripplot}}, \code{\link[lattice]{panel.bwplot}}, \code{\link[lattice]{panel.violin}}, \code{\link[vcd]{strucplot}}, \code{\link[barcode]{barcode}}
#' @examples
#' data(ais)
#' sex   <- ais$sex
#' BMI   <- ais$BMI
#'
#' res   <- MoE_clust(ais[,3:7], G=3, gating= ~ BMI, expert= ~ sex, modelNames="VVE")
#' MoE_gpairs(res)
#'
#' # Produce the same plot with a violin plot in the lower triangle.
...
MoE_gpairs          <- function(res, response.type = c("points", "uncertainty", "density"), scatter.type = c("lm", "points"), conditional = c("stripplot", "boxplot"),
                                ...
                                gap = 0.05, buffer = 0.02, scatter.pars = list(...), stripplot.pars = list(...), barcode.pars = list(...), mosaic.pars = list(...), axis.pars = list(...), diag.pars = list(...), ...) {
  UseMethod("MoE_gpairs")
}

#' @method MoE_gpairs MoEClust
#' @export
MoE_gpairs.MoEClust <- function(res, response.type = c("points", "uncertainty", "density"), scatter.type = c("lm", "points"), conditional = c("stripplot", "boxplot"),
                                addEllipses = c("outer", "yes", "no", "inner"), border.col = c("purple", "black", "brown", "brown", "navy"), bg.col = c("cornsilk", "white", "whitesmoke", "whitesmoke", "cornsilk"),
                                diagonal = c("on", "off"), outer.margins = list(bottom=unit(2, "lines"), left=unit(2, "lines"), top=unit(2, "lines"), right=unit(2, "lines")), outer.labels = NULL, outer.rot = c(0, 90),
                                gap = 0.05, buffer = 0.02, scatter.pars = list(...), stripplot.pars = list(...), barcode.pars = list(...), mosaic.pars = list(...), axis.pars = list(...), diag.pars = list(...), ...) {
  ...
  uni.c <- unique(class)
  class <- factor(class, levels=uni.c, labels=if(is.null(res$parameters$Vinv)) Gseq else replace(uni.c, uni.c > 0, Gseq))
  x     <- cbind(MAP=class, dat, net)
  ...
  if(is.null(scatter.pars$lcol))     {
    scatter.pars$lcol   <- colors[match(Gseq, uni.c)]
  }
  ...
  for(i   in Nseq) {
    for(j in Nseq) {
      ...
      if(i == j) {
        .diag_panel(x=x[,i], varname=names(x)[i], diag.pars=diag.pars, hist.col=if(i == 1 && names(x)[i] == "MAP") list(colors) else diag.pars$hist.color, axis.pars=axis.pars, xpos=xpos, ypos=ypos, buffer=buffer, index=i, outer.rot=outer.rot)
      } else {
        if(is.factor(x[,i]) + is.factor(x[,j]) == 1) {
          if(i < j & upr.cond != "barcode") .boxplot_panel(x=x[,j], y=x[,i], type=upr.cond, axis.pars=axis.pars, xpos=xpos, ypos=ypos, buffer=buffer, stripplot.pars=stripplot.pars, outer.rot=outer.rot, bg=bg, box.fill=if(i == 1 || j == 1) colors else if(i > dcol && j > dcol) "grey" else NULL, border=border)
          if(i > j & low.cond != "barcode") .boxplot_panel(x=x[,j], y=x[,i], type=low.cond, axis.pars=axis.pars, xpos=xpos, ypos=ypos, buffer=buffer, stripplot.pars=stripplot.pars, outer.rot=outer.rot, bg=bg, box.fill=if(i == 1 || j == 1) colors else if(i > dcol && j > dcol) "grey" else NULL, border=border)
        ...
        if(!any(is.factor(x[,i]), is.factor(x[,j]))) {
          .scatter_panel(x=x[,j], y=x[,i], type=ifelse(j > dcol && i <= dcol, upr.exp, ifelse(j <= dcol && i <= dcol, ifelse(drawEllipses, "ellipses", "points"), low.exp)),
                         scatter.pars=scatter.pars, axis.pars=axis.pars, xpos=xpos, ypos=ypos, buffer=buffer, z=z, G=G, res=res, dimens=c(j - 1, i - 1), outer.rot=outer.rot, bg=bg,
                         uncertainty=if(response.type == "uncertainty" && (i <= dcol && j <= dcol)) uncertainty else NA, mvn.col=if(colEllipses) scatter.pars$lcol else NULL, mvn.type=addEllipses, border=border)
        }
        if(all(is.factor(x[,i]),  is.factor(x[,j]))) {
          .mosaic_panel(x=x[,j], y=x[,i], mosaic.pars=mosaic.pars, mosaic.outer=if(j == 1) colors else if(i == 1) rep(colors, each=nlevels(x[,j])) else mosaic.pars$gp$col, axis.pars=axis.pars, xpos=xpos, ypos=ypos, outer.rot=outer.rot, bg=bg)
        }
      }
      popViewport(1)
      upViewport()
...
MoE_plotLogLik.MoEClust <- function(res, type = "l", xlab = "Iteration", ylab = "Log-Likelihood", xaxt = "n", ...) {
  ...
  plot(xll, type = type, xlab = xlab, ylab = ylab, xaxt = xaxt)
  if(length(xaxt) == 1  && is.character(xaxt)) {
    seqll    <- seq_along(xll)
    llseq    <- pretty(seqll)
    llseq    <- if(any(llseq  != floor(llseq))) seqll else llseq
    if(xaxt  == "n") axis(1, at = llseq, labels = llseq)
...
#' @importFrom graphics "axis" "matplot" "plot"
#' @importFrom barcode "barcode" "barcode.panel"
#' @importFrom grid "convertHeight" "convertWidth" "gpar" "grid.lines" "grid.newpage" "grid.points" "grid.polygon" "grid.rect" "grid.text" "grid.xaxis" "grid.yaxis" "popViewport" "pushViewport" "unit" "upViewport" "viewport"
#' @importFrom lattice "current.panel.limits" "panel.abline" "panel.barchart" "panel.bwplot" "panel.histogram" "panel.lines" "panel.points" "panel.stripplot" "panel.violin" "trellis.grobname" "trellis.par.get" "trellis.par.set"
#' @importFrom mclust "plot.mclustBIC" "plot.mclustICL"
...
.scatter_panel <- function(x, y, type, scatter.pars, axis.pars, xpos, ypos, buffer, z, G, res, dimens, outer.rot, bg, uncertainty, mvn.col, mvn.type, border) {
  ...
  if(type == "ellipses") {
    ...
    col=if(mvn.type == "inner") c("grey30", mvn.col[g], mvn.col[g]) else if(mvn.type == "outer") c(mvn.col[g], "grey30", "grey30"))
  }
  if(type == "lm") {
    for(g in seq_len(G)) {
...
.bar_code       <- function(x, outer.margins = list(bottom = unit(2, "lines"), left = unit(2, "lines"), top = unit(2, "lines"), right = unit(2, "lines")),
  ...
  barcode.panel(x, horizontal=horizontal, nint=nint, xlim=xlim, labelloc=labelloc, labelouter=labelouter,
