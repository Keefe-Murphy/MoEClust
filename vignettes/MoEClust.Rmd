...
### Installing MoEClust
__MoEClust__ will run in Windows, Mac OS X, or Linux. To install it you first need to install [R](https://cran.r-project.org/). Installing [RStudio](https://rstudio.com/) as a nice desktop environment for using R is also recommended.
...
x.new  <- setNames(seq(par("usr")[1L], par("usr")[2L], length=100L), rep("GNP", 100L))
y.new  <- setNames(seq(par("usr")[3L], par("usr")[4L], length=100L), rep("CO2", 100L))
