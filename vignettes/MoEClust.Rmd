...
Predictions can also be made from `MoEClust` models: the response, probability of cluster membership, and the MAP classification can be predicted for the fitted data or for new data (in the form of new covarariates and new response variables, or new covariates only). Let's predict the MAP classification for the optimal model fit above to the CO2 data.

```{r}
predict(comp$optimal)$classification
```
...
Replace the scatter plots in response vs. response panels with bivariate density contours. Note that this is liable to slow for models with expert network covariates.
