...
Fit various mixture models to cluster the CO2 data, allowing the GNP variable enter the gating &/or expert networks, or neither. Initialise the allocations randomly.

```{r, results='hide', eval=FALSE}
m1    <- MoE_clust(CO2, G=1:2, verbose=FALSE, init.z="random")
m2    <- MoE_clust(CO2, G=1:2, gating= ~ GNP, verbose=FALSE, init.z="random")
m3    <- MoE_clust(CO2, G=1:2, expert= ~ GNP, verbose=FALSE, init.z="random")
m4    <- MoE_clust(CO2, G=1:2, gating= ~ GNP, expert= ~ GNP, verbose=FALSE, init.z="random")
m5    <- MoE_clust(CO2, G=1:2, equalPro=TRUE, verbose=FALSE, init.z="random")
m6    <- MoE_clust(CO2, G=1:2, expert= ~ GNP, equalPro=TRUE, verbose=FALSE, init.z="random")
```
...
```{r, echo=FALSE}
res    <- comp$optimal
plot(x=GNP, y=CO2, main=substitute(atop(paste('CO'[2], " Data"), paste(Mname, ": ", rG, " components  (incl. expert network covariate: GNP)")), list(Mname=res$modelName, rG=res$G)), ylab=expression('CO'[2]), type="n")
for(k in seq_len(res$G)) {
  ck    <- res$classification == k
  text(GNP[ck], CO2[ck], CO2data$country[ck], col=k, cex=res$z[ck,k])
  expX <- res$expert[[k]]
...
plot(x=x, y=y, main=substitute(atop(paste('CO'[2], " Data"), paste(Mname, ": ", rG, " components  (incl. expert network covariate: GNP)")), list(Mname=res$modelName, rG=G)), ylab=expression('CO'[2]), type="n")
...
```{r, echo=FALSE}
(mod2 <- as.Mclust(mod))
plot(mod2, what="BIC", legendArgs=list(cex=0.8))
```
